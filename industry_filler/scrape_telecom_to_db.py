"""
HH.uz Telekommunikatsiya sektoridan kompaniyalarni DB ga saqlaydi.
"""
...
URLS = [
    ("Telekommunikatsiya (umumiy)", "https://tashkent.hh.uz/employers_company/telekommunikacii_svyaz"),
    ("Sun'iy yo'ldosh aloqasi", "https://tashkent.hh.uz/employers_company/telekommunikacii_svyaz/sputnikovaya_svyaz"),
    ("Qattiq aloqa", "https://tashkent.hh.uz/employers_company/telekommunikacii_svyaz/fiksirovannaya_svyaz"),
    ("Mobil aloqa", "https://tashkent.hh.uz/employers_company/telekommunikacii_svyaz/mobilnaya_svyaz"),
    ("Optik tolali aloqa", "https://tashkent.hh.uz/employers_company/telekommunikacii_svyaz/optovolokonnaya_svyaz"),
]
...
def save_to_db(companies: list[dict]):
    ...
    for c in companies:
        ...
        if cur.fetchone():
            ...
            """, (c["id"], c["name"], "Telekommunikatsiya", "Телекоммуникации", "Telecommunications", "61", 95, "hh_catalog"))
            added += 1
    conn.commit()
    conn.close()
...
def main():
    all_companies = {}
    print("HH.uz Telekommunikatsiya — yuklanmoqda...\n")
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM company_classifications WHERE industry_uz='Telekommunikatsiya'")
