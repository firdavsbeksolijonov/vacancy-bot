"""
HH.uz Davlat tashkilotlaridan kompaniyalarni DB ga saqlaydi.
"""
...
URLS = [
    ("Davlat tashkilotlari (umumiy)", "https://tashkent.hh.uz/employers_company/gosudarstvennye_organizacii"),
    ("Davlat tashkilotlari", "https://tashkent.hh.uz/employers_company/gosudarstvennye_organizacii/gosudarstvennye_organizacii"),
]
...
def save_to_db(companies: list[dict]):
    ...
    for c in companies:
        ...
        if cur.fetchone():
            ...
            """, (c["id"], c["name"], "Davlat tashkilotlari", "Государственные организации", "Government", "84", 95, "hh_catalog"))
            added += 1
    conn.commit()
    conn.close()
...
def main():
    all_companies = {}
    print("HH.uz Davlat tashkilotlari — yuklanmoqda...\n")
    ...
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) FROM company_classifications WHERE industry_uz='Davlat tashkilotlari'")
