...
- feedparser orqali RSS o'qiydi
- SQLite DB da ko'rilganlarni saqlaydi
- Industry ID bo'yicha filtr qiladi
- INCLUDE/EXCLUDE filtrlar
- Til va tajriba filtri
- Har sohadan max 2 ta
- Oxirida xulosa xabari
"""
...
MAX_PER_INDUSTRY = 2   # Har sohadan max shuncha vakansiya
MAX_AGE_DAYS = 3       # Shundan eski vakansiyalar o'tkazib yuboriladi
MAX_WORKERS = 8        # Parallel so'rovlar soni
...
INDUSTRIES = {
    ...
    "💻 IT / Texnologiya": {
        ...
        "keywords": [
            ...
            "project manager IT", "техническй менеджер",
        ],
    },
    "👥 HR / Rekruting": {
        "ids": ["27"],
...
# ✅ Sarlavhada shu so'zlardan biri bo'lsin
INCLUDE_TITLES = [
...
# ❌ Sarlavhada shu so'zlar bo'lsa o'tkazib yuboriladi
EXCLUDE_TITLES = [
    # Noto'g'ri kasblar
...
def is_title_ok(title):
    t = title.lower()
    # Avval EXCLUDE tekshiramiz
    for word in EXCLUDE_TITLES:
        if word.lower() in t:
            return False
    # Keyin INCLUDE tekshiramiz
...
def is_desc_ok(desc):
    d = desc.lower()
    # 5+ yil tajriba talab qilsa o'tkazib yuboramiz
    high_exp = re.search(
...
def build_rss_url(industry_ids, keyword):
    """Bitta kalit so'z + industry ID lar bilan RSS URL yasaydi."""
...
def process():
    ...
    for industry_name, cfg in INDUSTRIES.items():
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as ex:
            futures = {
                ex.submit(
                    fetch_for_keyword, kw, industry_name, industry_ids
                ): kw
                for kw in keywords
            }
        ...
        sent = 0
        for v in collected:
            if sent >= MAX_PER_INDUSTRY:
                break
            if is_seen(cursor, v["id"]):
                continue
        ...
        print(f"  {industry_name}: {len(collected)} topildi → {sent} ta yuborildi")
    ...
    print(f"⏱  Keyingi tekshiruv: {CHECK_INTERVAL // 60} daqiqadan keyin\n")
